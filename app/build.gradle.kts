plugins {
    id("com.android.application")
}

android {
    namespace = "com.akapps.dailynews"
    compileSdk = 34

    defaultConfig {
        applicationId = "com.akapps.dailynews"
        minSdk = 22
        targetSdk = 34
        versionCode = 1
        versionName = "1.0"

        testInstrumentationRunner = "androidx.test.runner.AndroidJUnitRunner"
    }

    buildTypes {
        release {
            isMinifyEnabled = false
            proguardFiles(
                getDefaultProguardFile("proguard-android-optimize.txt"),
                "proguard-rules.pro"
            )
        }
    }
    compileOptions {
        sourceCompatibility = JavaVersion.VERSION_1_8
        targetCompatibility = JavaVersion.VERSION_1_8
    }
}

dependencies {

    implementation("androidx.appcompat:appcompat:1.6.1")
    implementation("com.google.android.material:material:1.11.0")
    implementation("androidx.constraintlayout:constraintlayout:2.1.4")
    testImplementation("junit:junit:4.13.2")
    androidTestImplementation("androidx.test.ext:junit:1.1.5")
    androidTestImplementation("androidx.test.espresso:espresso-core:3.5.1")


    implementation("androidx.lifecycle:lifecycle-livedata-ktx:2.7.0") //2.5.1
    implementation("androidx.lifecycle:lifecycle-viewmodel-ktx:2.7.0")  //2.5.1
    implementation("androidx.navigation:navigation-fragment:2.7.7")  //2.5.3
    implementation("androidx.navigation:navigation-ui:2.7.7")  //2.5.3
    implementation("com.android.support:multidex:1.0.3")
    implementation("com.shreyaspatil:MaterialNavigationView:1.2")
}